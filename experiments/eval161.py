...
b["rmse"]=g("exp161_v7rmse_oof.csv.gz")
...
print("=== exp161: v7 구조 + RMSE(=Brier) 손실 — 다양성 멤버 추가 판정 ===\n")
for yr in (2024,2022):
    s=d[d.season==yr]; y=s[TARGET].to_numpy()
    v7,nn,rm=s.v7.to_numpy(),s.nn.to_numpy(),s.rmse.to_numpy()
    base=0.55*v7+0.45*nn
    print(f"[{yr}]  v7(Logloss) {sc(v7,y):.8f}   v7(RMSE) {sc(rm,y):.8f}   차 {(sc(v7,y)-sc(rm,y))*1e5:+.2f}e-5")
