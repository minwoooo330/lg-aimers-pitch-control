# -*- coding: utf-8 -*-
"""실험 03: 원본 피처 HGB 공통 시간 검증.

각 회차(fold)에서 과거 연도로만 학습하고 바로 다음 연도를 시험한다.
파생 피처를 추가하지 않은 공식 원본 입력 47개를 사용한다.
"""
...
RESULT_PATH = RESULT_DIR / "exp03_hgb_raw_walkforward.csv"
OOF_PATH = RESULT_DIR / "exp03_hgb_raw_oof.csv.gz"
...
def main():
    ...
    print(f"전체 {len(df):,}행, 원본 입력 {len(feature_cols)}개")
    ...
    for fold_name, valid_year in FOLDS:
        ...
        categorical_mask = [c in CAT_COLS for c in feature_cols]
