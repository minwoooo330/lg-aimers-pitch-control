...
def add_season_todate(d, tbl):
    """수치 안전판: 당해 표본 20구 미만이면 NaN(모델이 결측으로 처리), 비율은 [0,1] 클립."""
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp103b_season_todate.csv",index=False,encoding="utf-8-sig")
    for sd_on in [False,True]:
        pd.concat([store[(y,sd_on)] for y in [2024,2022] if (y,sd_on) in store],ignore_index=True)\
          .to_csv(RES/f"exp103b_{'sd' if sd_on else 'base'}_oof.csv.gz",index=False,compression="gzip")
    r=pd.DataFrame(rows)
    print()
    for year in [2024,2022]:
