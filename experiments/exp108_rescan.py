# -*- coding: utf-8 -*-
"""새 챔피언(sd 20% 포함) 기준 재스캔: sd 비중 최적화 + 109개 재료 한계기여 재계산."""
...
sd=col("exp105_sd_oof.csv.gz"); msk=np.isfinite(sd)
def ev(p,yr):
    m=(season==yr)&rel
    q=p[m]+(y[m].mean()-p[m].mean()); return float(np.mean((q-y[m])**2))
def mk(w):
    p=sharp.copy(); p[msk]=(1-w)*sharp[msk]+w*sd[msk]; return p
champ=mk(0.20)                      # 현 챔피언 = 1020.8774
b24,b22=ev(champ,2024),ev(champ,2022)
print(f"현 챔피언(sd 20%)  2024 {b24:.8f}  2022 {b22:.8f}\n")
print("=== sd 비중 재탐색 (현 챔피언 대비) ===")
for w in [0.20,0.25,0.30,0.35,0.40]:
    p=mk(w); sc_,sh=ec.score(p,y,season,rel)
    print(f"  sd {int(w*100)}%:  2024 {(b24-ev(p,2024))*1e5:+6.2f}  2022 {(b22-ev(p,2022))*1e5:+6.2f}  shift {sh:.12f}")
mats={}
...
for nm,v in mats.items():
    ...
    rows.append((nm,d24,d22,float(np.corrcoef(champ[m2],v[m2])[0,1])))
r=pd.DataFrame(rows,columns=["재료","d2024","d2022","상관"]).sort_values("d2024",ascending=False)
r.to_csv(RES/"exp108_rescan.csv",index=False,encoding="utf-8-sig")
print("\n=== 새 챔피언에 10% 추가 시 (2024·2022 둘 다 양수) ===")
