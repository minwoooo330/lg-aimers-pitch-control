...
def main():
    ...
    SEEDS=[42,7,2024]
    CONFIGS=[("B_8ep_코사인",8,True),("A_4ep_고정",4,False)]
    year=2022
    for name,eps,cos in CONFIGS:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp109_B_2022.csv",index=False,encoding="utf-8-sig")
        store[name].to_csv(RES/f"exp109_{name}_2022_oof.csv.gz",index=False,compression="gzip")
