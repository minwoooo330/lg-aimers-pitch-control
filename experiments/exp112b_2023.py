...
def main():
    ...
    for year in [2023]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp112b_2023.csv",index=False,encoding="utf-8-sig")
        for name in ["cat_sd"]:
            pd.concat([store[(y,name)] for y in [2023] if (y,name) in store],ignore_index=True)\
              .to_csv(RES/f"exp112b_{name}_2023_oof.csv.gz",index=False,compression="gzip")
