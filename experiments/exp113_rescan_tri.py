# -*- coding: utf-8 -*-
"""새 챔피언(tri_v1 = sd40 + nn8 10% + sdall 10%, 1033.2280) 기준 전면 재스캔."""
...
sdb=col("exp110_sd_pit_bat_oof.csv.gz"); sda=col("exp110_sd_all_oof.csv.gz")
...
m2=np.isfinite(nn8); p[m2]=0.90*p[m2]+0.10*nn8[m2]
...
print(f"tri 챔피언(1033.2280)  2024 {b24:.8f}  2022 {b22:.8f}\n")
mats={}
...
mats["NN8ep_cos"]=nn8
mats["sean_v42"]=b.row_id.map(pd.read_csv(B/"sean_v42_oof.csv.gz").set_index("row_id")["prediction"]).to_numpy()
...
mats["TEAM_extratrees"]=b.row_id.map(ex.set_index("row_id")["prediction"]).to_numpy()
rows=[]
...
r.to_csv(RES/"exp113_rescan_tri.csv",index=False,encoding="utf-8-sig")
print("=== 둘 다 양수 ===")
...
print("\n=== 비중 재탐색 (sd/nn8/sdall) ===")
for wsd in [0.40,0.45,0.50]:
    for wn in [0.10,0.15]:
        q=sharp.copy(); q[m1]=(1-wsd)*sharp[m1]+wsd*sdb[m1]
        q[m2]=(1-wn)*q[m2]+wn*nn8[m2]; q[m3]=0.90*q[m3]+0.10*sda[m3]
        sc_,sh=ec.score(q,y,season,rel)
        print(f"  sd{int(wsd*100)}+nn{int(wn*100)}+all10: 2024 {(b24-ev(q,2024))*1e5:+6.2f}  2022 {(b22-ev(q,2022))*1e5:+6.2f}  shift {sh:.12f}")
