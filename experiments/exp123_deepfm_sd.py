...
class DeepFM(nn.Module):
    """FM 브랜치: 범주형 필드 9개의 쌍별 상호작용을 저랭크(dim=8) 내적으로 명시적 모델링.
       Deep 브랜치: 기존 성공 구조(손잡이분할 임베딩 + MLP) 그대로."""
    def __init__(self,sizes,ndim):
        ...
        self.mlp=nn.Sequential(nn.Linear(dim,256),nn.ReLU(),nn.Dropout(0.15),
                               ...
                               nn.Linear(128,1))
        # FM 전용 임베딩: 기본 초기화(N(0,1))는 쌍별 상호작용 항을 초기부터 크게 만들어 불안정.
        # 작은 표준편차로 초기화해 학습 초반 FM 기여를 작게 시작한다.
    ...
    def forward(self,xc,xn,sm):
        ...
        deep_logit=self.mlp(torch.cat(e+[ph,xn],dim=1)).squeeze(1)
        # FM: 0.5*((sum v_i)^2 - sum v_i^2)  , 필드별 임베딩 벡터의 쌍별 내적 합을 O(F*d)로 계산
        fv=torch.stack([emb(xc[:,j]) for j,emb in enumerate(self.fm_embs)],dim=1)  # (B,F,d)
        sum_sq=fv.sum(dim=1).pow(2).sum(dim=1)          # (B,)
        sq_sum=fv.pow(2).sum(dim=1).sum(dim=1)           # (B,)
        fm_inter=0.5*(sum_sq-sq_sum)
        fm_lin=sum(lin(xc[:,j]).squeeze(1) for j,lin in enumerate(self.fm_linear))
        fm_logit=fm_lin+fm_inter
        return deep_logit+fm_logit   # 초기화로 이미 작으므로 추가 스케일 불필요, 학습이 알아서 키움
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp123_deepfm_sd_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp123_deepfm_sd.csv",index=False,encoding="utf-8-sig")
