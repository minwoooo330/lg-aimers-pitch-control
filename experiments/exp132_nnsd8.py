# -*- coding: utf-8 -*-
"""exp132 — nnsd(챔피언 최대 성분, 비중 45%)에 8에폭+코사인 스케줄 적용.

  동기: 점수 상승 경로는 "새 정보 채널"만이 아니다. 고비중 멤버의 품질을 올리는 경로는
        상관계수와 무관하게 작동하며 실제로 여러 번 통했다(시드평균 +4.11/+8.58, nn8 +5.27).
  발견: 8에폭+코사인은 nn8 도입 때 +5.27점을 낸 검증된 처방인데, 정작 비중이 3배 큰
        nnsd는 4에폭 고정에 스케줄도 없는 구버전 레시피로 돌고 있었다.
  정황: nnsd 시드 스프레드가 크다(2024 개별 0.24840/0.24833/0.24835 -> 3시드평균 0.24799).
        평균이 개별보다 37~40e-5나 좋다는 것은 잡음이 많이 남아 학습이 덜 수렴했다는 신호다.
  판정: 체인 한계기여로 한다. 교체형(nnsd -> nnsd8)과 추가형 둘 다 측정한다."""
...
def run_fold(df,year,seed,tables):
    ...
    _spe=(len(tr)+BATCH-1)//BATCH
    sched=torch.optim.lr_scheduler.CosineAnnealingLR(opt,T_max=EPOCHS*_spe)
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp132_nnsd8_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp132_nnsd8.csv",index=False,encoding="utf-8-sig")
