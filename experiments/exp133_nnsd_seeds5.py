# -*- coding: utf-8 -*-
"""exp133 — nnsd(챔피언 최대 성분 45%) 시드 5개 추가 -> 8시드 평균.

  exp132에서 8에폭+코사인은 과학습으로 기각됐다(2024 -32.5e-5). 에폭은 현행 4를 유지한다.
  남은 경로 B: 시드 평균 확대. nnsd의 시드 잡음이 크다 —
  2024 개별 시드 평균 0.24835706 vs 3시드 평균 0.24798621 (격차 37.1e-5).
  총 잡음분산 V=55.6e-5로 추정되어 3->8 시드면 이 모델에서 +11.6e-5 여지가 있다.
  선례 주의: 손잡이NN 3->8 시드는 단독 개선에도 체인 기여 -0.05e-5(포화)였다.
  다만 그 멤버는 비중 25%였고 nnsd는 45%라 실측이 필요하다."""
...
def main():
    ...
    for year in [2024,2022]:
        ...
        for _s in SEEDS: parts_seed.setdefault(_s,[]).append(
            pd.DataFrame({ID:ids,"season":year,"prediction":ps[_s]}))
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp133_nnsd_seeds5_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp133_nnsd_seeds5.csv",index=False,encoding="utf-8-sig")
        for _s,_p in parts_seed.items():
            pd.concat(_p,ignore_index=True).to_csv(RES/f"exp133_seed{_s}_oof.csv.gz",index=False,compression="gzip")
