# -*- coding: utf-8 -*-
"""exp137 — NN + sd + 휴리스틱 56개. sd 계열 NN이 한 번도 본 적 없는 피처군.
  exp96에서 HGB로 시험해 기각(2024 +2.99 / 2022 -3.32)했으나 NN에는 미시도.
  근거: 오늘 순위 피처가 HGB +0.08(~0) -> NN +0.98/+0.45로 통과했다.
  HGB 스크리닝은 NN에 대해 거짓 음성을 낸다는 것이 실증됐으므로 재시험한다."""
...
def run_fold(df,year,seed,tables):
    ...
    xn_tr=pd.concat([tr[num_cols],add_features(tr),add_sd(tr,tables),add_hfeatures(tr)],axis=1).astype(np.float32)
    xn_va=pd.concat([va[num_cols],add_features(va),add_sd(va,tables),add_hfeatures(va)],axis=1).astype(np.float32)
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp137_nn_hfeat_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp137_nn_hfeat.csv",index=False,encoding="utf-8-sig")
