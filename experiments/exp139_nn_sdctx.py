# -*- coding: utf-8 -*-
"""exp139 — NN + sd + (폼편차 x 상황) 상호작용. exp128을 NN으로 재시험.
  exp128은 HGB에서 2024 -2.92 / 2022 +9.45로 부호가 갈려 기각됐다.
  2022에서 2.8sigma 실신호가 있었으므로 NN에서 살아날 여지를 본다."""
...
_s2=_u.spec_from_file_location("sx",Path(__file__).resolve().parent/"exp128_sd_x_context.py")
sx=_u.module_from_spec(_s2); _s2.loader.exec_module(sx)
...
def run_fold(df,year,seed,tables):
    ...
    xn_tr=pd.concat([tr[num_cols],add_features(tr),add_sd(tr,tables),sx.add_sd_x_context(tr,add_sd(tr,tables))],axis=1).astype(np.float32)
    xn_va=pd.concat([va[num_cols],add_features(va),add_sd(va,tables),sx.add_sd_x_context(va,add_sd(va,tables))],axis=1).astype(np.float32)
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp139_nn_sdctx_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp139_nn_sdctx.csv",index=False,encoding="utf-8-sig")
