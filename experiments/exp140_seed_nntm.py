# -*- coding: utf-8 -*-
"""exp140_seed_nntm — nntm 시드 5개 추가 -> 8시드 평균.
  근거: 오늘 nnsd를 3->8 시드로 확장하니 체인 기여 2024 +1.74 / 2022 +1.39가 나왔다.
  hepta에 새로 들어간 nntm는 아직 시드 3개다. 같은 처방을 적용한다.
...
def run_fold(df,year,seed,tables,tmprof):
    ...
    xn_tr=pd.concat([tr[num_cols].reset_index(drop=True),add_features(tr).reset_index(drop=True),
                     add_sd(tr,tables).reset_index(drop=True),tm_tr],axis=1).astype(np.float32)
    xn_va=pd.concat([va[num_cols].reset_index(drop=True),add_features(va).reset_index(drop=True),
                     add_sd(va,tables).reset_index(drop=True),tm_va],axis=1).astype(np.float32)
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp140_seed_nntm_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp140_seed_nntm.csv",index=False,encoding="utf-8-sig")
