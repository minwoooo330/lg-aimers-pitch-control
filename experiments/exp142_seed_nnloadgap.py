# -*- coding: utf-8 -*-
"""exp142_seed_nnloadgap — nnloadgap 시드 5개 추가 -> 8시드 평균.
  근거: 오늘 nnsd를 3->8 시드로 확장하니 체인 기여 2024 +1.74 / 2022 +1.39가 나왔다.
  hepta에 새로 들어간 nnloadgap는 아직 시드 3개다. 같은 처방을 적용한다.
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp142_seed_nnloadgap_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp142_seed_nnloadgap.csv",index=False,encoding="utf-8-sig")
