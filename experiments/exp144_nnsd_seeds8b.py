# -*- coding: utf-8 -*-
"""exp144 — nnsd 시드 8개 추가 -> 16시드 평균.

  nnsd는 챔피언 최대 성분(실효가중 31%)이고, 시드 확장이 유일하게 반복 성공한 자리다
  (3->8에서 체인 2024 +1.74 / 2022 +1.39).
  다른 멤버(nntm/nnrank/nnloadgap)는 3->8에서 전부 포화였다 — 가중치가 7.7~15%로 낮아서다.
  분산감소 이론상 8->16의 기대 기여는 3->8의 0.30배 = 2024 약 +0.52e-5.
  16시드 평균 = (8*기존avg8 + 8*신규avg8)/16 으로 정확히 계산 가능."""
...
EPOCHS=4; BATCH=8192; SEEDS=[3, 21, 55, 90, 123, 256, 512, 999]
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp144_nnsd_seeds8b_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp144_nnsd_seeds8b.csv",index=False,encoding="utf-8-sig")
