# -*- coding: utf-8 -*-
"""exp150 — nnsd 16시드의 2023 fold. 팀원 OOF 교환용(3-fold 완전판)."""
...
EPOCHS=4; BATCH=8192; SEEDS=[42,7,2024,1,13,77,101,777,3,21,55,90,123,256,512,999]
...
def main():
    ...
    for year in [2023]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp150_nnsd16_2023_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp150_nnsd16_2023.csv",index=False,encoding="utf-8-sig")
