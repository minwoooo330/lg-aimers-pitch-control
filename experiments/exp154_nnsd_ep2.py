# -*- coding: utf-8 -*-
"""exp154 — nnsd를 2에폭 x 16시드로. exp153에서 검증 최저가 2에폭임을 확인(현행 4에폭은 16e-5 손해).
   시드평균이 과적합 잡음을 일부 상쇄하므로, 16시드에서도 2에폭이 유리한지 직접 확인한다."""
...
EPOCHS=2; BATCH=8192; SEEDS=[42,7,2024,1,13,77,101,777,3,21,55,90,123,256,512,999]
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp154_nnsd_ep2_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp154_nnsd_ep2.csv",index=False,encoding="utf-8-sig")
