# -*- coding: utf-8 -*-
"""exp155 — label smoothing(0.05) + 2에폭 x 8시드.
   exp153에서 과적합이 확정됐다(학습 Brier 단조감소, 검증은 2에폭 이후 상승).
   에폭 축소는 exp154에서 보고, 여기서는 타깃 평활이 추가로 도움이 되는지 본다.
   y를 0/1 대신 0.05/0.95로 학습해 극단 확신을 억제한다."""
...
LS=0.05
EPOCHS=2; BATCH=8192; SEEDS=[42,7,2024,1,13,77,101,777]
...
def run_fold(df,year,seed,tables):
    ...
    for ep in range(EPOCHS):
        ...
        for st in range(0,n,BATCH):
            ...
            lossf(net(Xc[bb],Xn[bb],S[bb]),Y[bb]*(1-2*LS)+LS).backward(); opt.step()
    net.eval(); ps=[]
    with torch.no_grad():
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp155_nnsd_ls_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp155_nnsd_ls.csv",index=False,encoding="utf-8-sig")
