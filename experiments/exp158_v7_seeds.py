# -*- coding: utf-8 -*-
"""exp158 — v7 OOF에 시드 2개(7, 2024) 추가. exp148(시드42)과 합쳐 3시드 OOF를 만들어
   시드 확장이 실제로 혼합에 이득인지 검증한다. 배포본은 이미 9시드를 학습해 뒀다."""
...
def main():
    ...
    for year in [2024,2022]:
        ...
        for spec in V7.ENSEMBLE:
            for sd_ in EXTRA_SEEDS:
                t1=time.time()
                nm,dep,it,lr,_,_=spec
                m=V7.make_model((nm,dep,it,lr,L2,RSM),sd_,thread_count=8)
                m.set_params(monotone_constraints=mono)
                m.fit(pool)
                preds.append(m.predict_proba(Xv)[:,1])
                print(f"  cat_{nm}_s{sd_} ({time.time()-t1:.0f}s)",flush=True)
                del m; gc.collect()
        ...
        pd.concat(parts_oof,ignore_index=True).to_csv(RES/"exp158_v7_seed7_2024_oof.csv.gz",index=False,compression="gzip")
