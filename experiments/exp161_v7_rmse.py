# -*- coding: utf-8 -*-
"""exp161 — v7 구조를 Logloss 대신 RMSE(={0,1}에서 Brier와 동일) 손실로 학습.
   교체가 아니라 다양성 멤버 추가가 목적. 2024 fold 먼저 관문."""
...
def main():
    ...
    for year in [2024,2022]:
        ...
        for spec in V7.ENSEMBLE:
            ...
            m=CatBoostRegressor(iterations=it,learning_rate=lr,depth=dep,l2_leaf_reg=L2,
                                rsm=RSM,loss_function="RMSE",random_seed=SEED,verbose=0,
                                thread_count=8,monotone_constraints=mono)
            m.fit(pool)
            preds.append(np.clip(m.predict(Xv),1e-4,1-1e-4))
        ...
        pd.concat(parts_oof,ignore_index=True).to_csv(RES/"exp161_v7rmse_oof.csv.gz",index=False,compression="gzip")
