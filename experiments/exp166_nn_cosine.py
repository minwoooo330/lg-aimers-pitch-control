# -*- coding: utf-8 -*-
"""exp166 — NN 학습 절차 현대화. 코사인 LR 스케줄 + 에폭 확장.
   현행은 4에폭 상수LR 무스케줄. NN은 최종 혼합의 45%.
   EP, COS 환경변수로 제어."""
...
EPOCHS=int(os.environ.get("EP","8")); BATCH=8192; SEEDS=[42,7,2024]
COS=int(os.environ.get("COS","1"))
...
def run_fold(df,year,seed,tables):
    ...
    sched=torch.optim.lr_scheduler.CosineAnnealingLR(opt,T_max=steps) if COS else None
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/f"exp166_nncos_ep{EPOCHS}_cos{COS}_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp166_nncos.csv",index=False,encoding="utf-8-sig")
