# -*- coding: utf-8 -*-
"""exp167 — NN 에폭 수 스윕 (스케줄 없음, 상수 LR 유지).
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/f"exp167_ep{EPOCHS}_cos{COS}_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp167_epochs.csv",index=False,encoding="utf-8-sig")
