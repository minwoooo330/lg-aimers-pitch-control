# -*- coding: utf-8 -*-
"""exp168 — v7에 raw 선수 ID 2컬럼 추가 게이트. 백지 세션 발견(CatBoost에서 raw ID 유효) 기반.
   v7은 ID를 완전히 제거하고 carry 테이블로 대체했는데 그 대체가 완전한지 검증.
   d7 단일 설정, 시드42, ID 유/무 짝지음."""
...
def main():
    ...
    for year in [2024,2022]:
        ...
        Xi=X.copy(); Xvi=Xv.copy()
        Xi["pid_raw"]=df["pitcher_id"].to_numpy(np.float32); Xi["bid_raw"]=df["batter_id"].to_numpy(np.float32)
        Xvi["pid_raw"]=va["pitcher_id"].to_numpy(np.float32); Xvi["bid_raw"]=va["batter_id"].to_numpy(np.float32)
        mono_i=mono+[0,0]
        for tag,XX,XXv,mm in [("no_id",X,Xv,mono),("raw_id",Xi,Xvi,mono_i)]:
            t1=time.time()
            nm,dep,it,lr,_,_=V7.ENSEMBLE[0]   # d7
            m=V7.make_model((nm,dep,it,lr,L2,RSM),SEED,thread_count=8)
            m.set_params(monotone_constraints=mm)
            m.fit(Pool(XX,y,weight=w,cat_features=cats))
            preds[tag]=m.predict_proba(XXv)[:,1]
            print(f"  d7_{tag} ({time.time()-t1:.0f}s)",flush=True)
            del m; gc.collect()
        import pandas as _pd
        _pd.DataFrame({ID:va[ID].to_numpy(),"season":year,TARGET:yv,
                       "no_id":preds["no_id"],"raw_id":preds["raw_id"]}).to_csv(
            RES/f"exp168_rawid_{year}.csv.gz",index=False,compression="gzip")
        continue
