# -*- coding: utf-8 -*-
"""seoyeon_v6 ZIP에서 복원한 tuned LightGBM의 공통 3-fold OOF."""
...
def main():
 ...
 for year in [2022,2023,2024]:
  tr=df[df.season<year];va=df[df.season==year];xtr=add_interactions(tr[base]);xva=add_interactions(va[base]);nums=[c for c in xtr.columns if c not in CATS]
  ...
  model=Pipeline([("pre",pre),("clf",clf)]);ytr=tr[TARGET].to_numpy(np.int8);yva=va[TARGET].to_numpy(np.int8);t=time.time();model.fit(xtr,ytr);p=model.predict_proba(xva)[:,1];sec=time.time()-t;row={"model":"seoyeon_lgbm_rebuilt","fold":year,"brier":brier_score_loss(yva,p),"logloss":log_loss(yva,p),"roc_auc":roc_auc_score(yva,p),"pred_mean":p.mean(),"target_mean":yva.mean(),"seconds":sec};rows.append(row);parts.append(pd.DataFrame({ID:va[ID].to_numpy(),"season":year,TARGET:yva,"prediction":p}));print(row);del tr,va,xtr,xva,model,pre,clf,p;gc.collect()
 result=pd.DataFrame(rows);mean={"model":"seoyeon_lgbm_rebuilt","fold":"mean_2022_2024","brier":result.brier.mean(),"logloss":result.logloss.mean(),"roc_auc":result.roc_auc.mean(),"seconds":result.seconds.sum()};result=pd.concat([result,pd.DataFrame([mean])],ignore_index=True);RES.mkdir(exist_ok=True);result.to_csv(RES/"exp23_seoyeon_lgbm_walkforward.csv",index=False,encoding="utf-8-sig");pd.concat(parts,ignore_index=True).to_csv(RES/"exp23_seoyeon_lgbm_oof.csv.gz",index=False,compression="gzip");print(result.to_string(index=False));print(f"total={time.time()-start:.1f}s")
