# -*- coding: utf-8 -*-
"""실험 36: 임베딩 NN 3-fold 확장 (에폭 4 고정, 시드 42)."""
...
def main():
    ...
    d24=pd.read_csv(RES/"exp35_nn_2024_oof.csv.gz")
    d24["prediction"]=np.load(RES/"exp35_nn_2024_ep4.npy")
    parts.append(d24)
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp36_nn_oof.csv.gz",index=False,compression="gzip")
    pd.DataFrame(rows).to_csv(RES/"exp36_nn_walkforward.csv",index=False,encoding="utf-8-sig")
