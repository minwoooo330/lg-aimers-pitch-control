# -*- coding: utf-8 -*-
"""실험 39: asof 없는 NN 화면 (2024만). 임베딩+상황 정보만 -> 관점 극대화."""
...
def prep(df,year):
    ...
    num_cols=[c for c in df.columns if c not in cat_names and c not in (ID,TARGET)
              and not c.startswith("asof_")]
    xn_tr=tr[num_cols].astype(np.float32).copy()
    xn_va=va[num_cols].astype(np.float32).copy()
...
def main():
    ...
    pd.DataFrame({ID:ids,"season":2024,TARGET:yva.astype(np.int8),"prediction":avg}).to_csv(
        RES/"exp39_nn_noasof_2024_oof.csv.gz",index=False,compression="gzip")
