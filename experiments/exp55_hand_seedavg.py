# -*- coding: utf-8 -*-
"""실험 55: 손잡이 분리 NN 시드 3개(42/7/2024) 평균, 3-fold. 골라내기 없는 분산 축소."""
...
def main():
    ...
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp55_hand_seedavg_oof.csv.gz",index=False,compression="gzip")
    pd.DataFrame(rows).to_csv(RES/"exp55_hand_seedavg.csv",index=False,encoding="utf-8-sig")
