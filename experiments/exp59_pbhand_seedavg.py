# -*- coding: utf-8 -*-
"""실험 59: 타자 임베딩을 동손/반대손 2벌로 분리(손잡이 분할의 대칭판) + 시드 3개 평균, 3-fold."""
...
def run_fold(df,year,seed):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.b_same=nn.Embedding(sizes[1],16); self.b_opp=nn.Embedding(sizes[1],16)
            dim=xn_tr.shape[1]+sum(d for _,d in EMB)+32
        ...
        def forward(self,xc,xn,sm):
            ...
            bh=self.b_same(xc[:,1])*s+self.b_opp(xc[:,1])*(1-s)
            return self.mlp(torch.cat(e+[ph,bh,xn],dim=1)).squeeze(1)
...
def main():
    ...
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp59_pbhand_seedavg_oof.csv.gz",index=False,compression="gzip")
    pd.DataFrame(rows).to_csv(RES/"exp59_pbhand_seedavg.csv",index=False,encoding="utf-8-sig")
