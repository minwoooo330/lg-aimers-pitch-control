# -*- coding: utf-8 -*-
"""실험 66: 투수 임베딩을 (타자손잡이 2) x (카운트 우열 3) = 6벌로 분리한 NN, 시드 3개, 3-fold.
...
def run_fold(df,year,seed):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.pc=nn.ModuleList([nn.Embedding(sizes[1],16) for _ in range(6)])
        ...
        def forward(self,xc,xnum,cl):
            ...
            for g in range(6):
                m=(cl==g)
                if m.any(): ph[m]=self.pc[g](xc[m,1])
...
def main():
    ...
    for year in [2022,2023,2024]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp66_batcnt.csv",index=False,encoding="utf-8-sig")
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp66_batcnt_oof.csv.gz",index=False,compression="gzip")
