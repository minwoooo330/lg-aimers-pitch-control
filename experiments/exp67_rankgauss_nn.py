# -*- coding: utf-8 -*-
"""실험 67: 투수 임베딩을 (타자손잡이 2) x (카운트 우열 3) = 6벌로 분리한 NN, 시드 3개, 3-fold.
...
def run_fold(df,year,seed):
    ...
    qt=QuantileTransformer(output_distribution="normal",n_quantiles=1000,
                           subsample=200000,random_state=0).fit(xn_tr)
    xn_tr=qt.transform(xn_tr).astype(np.float32); xn_va=qt.transform(xn_va).astype(np.float32)
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.pc=nn.ModuleList([nn.Embedding(sizes[0],16) for _ in range(2)])
...
def main():
    ...
    for year in [2022,2023,2024]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp67_rankgauss.csv",index=False,encoding="utf-8-sig")
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp67_rankgauss_oof.csv.gz",index=False,compression="gzip")
