# -*- coding: utf-8 -*-
"""실험 68: 투수 임베딩을 (타자손잡이 2) x (카운트 우열 3) = 6벌로 분리한 NN, 시드 3개, 3-fold.
...
def run_fold(df,year,seed):
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.ph=nn.ModuleList([nn.Embedding(sizes[0],16) for _ in range(2)])
            self.pc=nn.ModuleList([nn.Embedding(sizes[0],16) for _ in range(3)])
            self.bh=nn.ModuleList([nn.Embedding(sizes[1],16) for _ in range(2)])
            dim=xn_tr.shape[1]+sum(d for _,d in EMB)+48
        ...
        def forward(self,xc,xnum,cl):
            ...
            sm=(cl//3); ad=(cl%3)
            a=torch.zeros(xc.shape[0],16); b_=torch.zeros(xc.shape[0],16); c_=torch.zeros(xc.shape[0],16)
            for g in range(2):
                m=(sm==g)
                if m.any():
                    a[m]=self.ph[g](xc[m,0]); c_[m]=self.bh[g](xc[m,1])
            for g in range(3):
                m=(ad==g)
                if m.any(): b_[m]=self.pc[g](xc[m,0])
            return self.mlp(torch.cat(e+[a,b_,c_,xnum],dim=1)).squeeze(1)
...
def main():
    ...
    for year in [2022,2023,2024]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp68_multisplit.csv",index=False,encoding="utf-8-sig")
    pd.concat(parts,ignore_index=True).to_csv(RES/"exp68_multisplit_oof.csv.gz",index=False,compression="gzip")
