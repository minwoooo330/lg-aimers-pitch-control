...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.DataFrame(rows).to_csv(RES/"exp80_heuristic_3seed.csv",index=False,encoding="utf-8-sig")
    for hf in [False,True]:
        pd.concat([store[(y,hf)] for y in [2024,2022] if (y,hf) in store],ignore_index=True)\
          .to_csv(RES/f"exp80_{'hf' if hf else 'base'}_avg3_oof.csv.gz",index=False,compression="gzip")
    r=pd.DataFrame(rows)
    for year in [2024,2022]:
