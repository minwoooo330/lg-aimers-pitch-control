# -*- coding: utf-8 -*-
"""실험 80: 휴리스틱 배치(hfeatures 50개) 3시드 재판정 — '닫지 않는다'.
...
def main():
    ...
    for year in [2023]:
        ...
        def enc(d,hf):
            ...
            if hf: parts.append(add_hfeatures(d))
        ...
        pd.DataFrame(rows).to_csv(RES/"exp80b_2023.csv",index=False,encoding="utf-8-sig")
    for hf in [False,True]:
        pd.concat([store[(y,hf)] for y in [2023] if (y,hf) in store],ignore_index=True)\
          .to_csv(RES/f"exp80b_{'hf' if hf else 'base'}_2023_oof.csv.gz",index=False,compression="gzip")
    r=pd.DataFrame(rows)
    for year in [2023]:
