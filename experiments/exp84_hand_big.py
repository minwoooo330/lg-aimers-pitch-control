...
def run_fold(df,year,seed):
    ...
    xn_tr=pd.concat([tr[num_cols],add_features(tr),add_hfeatures(tr)],axis=1).astype(np.float32)
    xn_va=pd.concat([va[num_cols],add_features(va),add_hfeatures(va)],axis=1).astype(np.float32)
    ...
    class Net(nn.Module):
        def __init__(self):
            ...
            self.mlp=nn.Sequential(nn.Linear(dim,384),nn.ReLU(),nn.Dropout(0.15),
                                   nn.Linear(384,192),nn.ReLU(),nn.Dropout(0.15),
                                   nn.Linear(192,1))
...
def main():
    ...
    for year in [2024,2022]:
        ...
        pd.concat(parts,ignore_index=True).to_csv(RES/"exp84_hand_big_oof.csv.gz",index=False,compression="gzip")
        pd.DataFrame(rows).to_csv(RES/"exp84_hand_big.csv",index=False,encoding="utf-8-sig")
