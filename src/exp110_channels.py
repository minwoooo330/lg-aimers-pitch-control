# -*- coding: utf-8 -*-
"""실험 110: sd 성공(+13.53점)을 다른 채널로 복제.
...
def main():
    ...
    for year in [2024,2022]:
        ...
        for name,tags in [("sd_pit",["pit"]),("sd_pit_bat",["pit","bat"]),
                          ("sd_all",["pit","pitbs","bat","mix"])]:
            xa,xb=enc(tr,tags),enc(va,tags)
        ...
        pd.DataFrame(rows).to_csv(RES/"exp110_channels.csv",index=False,encoding="utf-8-sig")
        for name in ["sd_pit","sd_pit_bat","sd_all"]:
            pd.concat([store[(y,name)] for y in [2024,2022] if (y,name) in store],ignore_index=True)\
              .to_csv(RES/f"exp110_{name}_oof.csv.gz",index=False,compression="gzip")
