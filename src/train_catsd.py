...
BASE=OUT/"kimminwoo_rcsharp_v1.zip"; ZIP=OUT/"kimminwoo_sdall_v1.zip"; PKL=OUT/"catsd_model.pkl"
ID,TARGET="row_id","control_success"
CAT=["top_bottom","game_type","base_state","pitcher_hand","batter_hand"]
SEEDS=[42]; RATES=["success_rate","middle_rate","reverse_rate"]
...
def main():
    ...
    x=df[base_cols].copy()
    for c in CA: x[c]=df[c].where(df[c].notna(),"__MISSING__").astype(str)
    X=pd.concat([x,add_features(df),add_hfeatures(df),sdf.reset_index(drop=True)],axis=1)
    idxc=[X.columns.get_loc(c) for c in CA]
